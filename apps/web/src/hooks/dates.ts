...
export function useFormattedDate(date: Date | string, options: Intl.DateTimeFormatOptions, locales: string | string[] | undefined = undefined): string {
  return React.useMemo(() => {
    return Intl.DateTimeFormat(locales, options).format(new Date(date));
  }, [date]);
}

export function useYearOptionalFormattedDate(date: Date | string, options: Omit<Intl.DateTimeFormatOptions, 'timeStyle' | 'dateStyle'>, locales: string | string[] | undefined = undefined): string {
